...
//           (json['rest_between_exercises_seconds'] as num).toInt(),
//       averageCaloriesBurnedPerMinute:
//           (json['average_calories_burned_per_minute'] as num).toInt(),
//       weeks: weeks,
//     );
//   }
// }
...
class WorkoutPlanModel {
  ...
  factory WorkoutPlanModel.fromJson(Map<String, dynamic> json) {
    var weeksList = json['weeks'] as List? ?? []; // Handle null case
    List<Week> weeks = weeksList.map((i) => Week.fromJson(i)).toList();
    ...
    return WorkoutPlanModel(
      ...
      estimatedDuration: json['estimated_duration'] ?? '',
      restBetweenExercisesSeconds:
          (json['rest_between_exercises_seconds'] as num?)?.toInt() ?? 0,
      averageCaloriesBurnedPerMinute:
          (json['average_calories_burned_per_minute'] as num?)?.toInt() ?? 0,
      weeks: weeks,
    );
  }
}
...
class Week {
  ...
  factory Week.fromJson(Map<String, dynamic> json) {
    var daysList = json['days'] as List? ?? []; // Handle null case
    List<Day> days = daysList.map((i) => Day.fromJson(i)).toList();
...
class Day {
  ...
  factory Day.fromJson(Map<String, dynamic> json) {
    var categoriesList = json['categories'] as List? ?? []; // Handle null case
    List<Category> categories =
        categoriesList.map((i) => Category.fromJson(i)).toList();
...
class Category {
  ...
  factory Category.fromJson(Map<String, dynamic> json) {
    var exercisesList = json['exercises'] as List? ?? []; // Handle null case
    List<Exercise> exercises =
        exercisesList.map((i) => Exercise.fromJson(i)).toList();

    return Category(
      subCategory: json['sub_category'] ?? '',
      circuitRestTime: (json['circuit_rest_time'] as num?)?.toInt() ?? 0,
      circuitReps: (json['circuit_reps'] as num?)?.toInt() ?? 0,
      exercises: exercises,
    );
  }
}
...
class Exercise {
  ...
  factory Exercise.fromJson(Map<String, dynamic> json) {
    return Exercise(
      name: json['name'] ?? '',
      exerciseNumber: (json['exercise_number'] as num?)?.toInt() ?? 0,
      exerciseType: json['exercise_type'] ?? '',
      timeBased: json['time_based'] ?? '',
      weighted: json['weighted'] ?? '',
      ...
      setTime: json['set_time'] ?? '',
      supersetNames: List<String>.from(json['superset_names'] ?? []),
      restTime: (json['rest_time'] as num?)?.toInt() ?? 0,
      videoUrl: json['video_url'] ?? '',
      imageUrl: json['image_url'] ?? '',
    );
  }
